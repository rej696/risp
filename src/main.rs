...
fn parse<'a>(tokens: &'a [String]) -> Result<(RispExp, &'a [String]), RispErr> {
    let (token, rest) = tokens.split_first().ok_or(RispErr::InvalidToken)?;
...
fn read_seq<'a>(tokens: &'a [String]) -> Result<(RispExp, &'a [String]), RispErr> {
    ...
    let mut xs = tokens;
    loop {
        let (next_token, rest) = xs.split_first().ok_or(RispErr::UnbalancedParens)?;
        if next_token == ")" {
...
fn eval_if_args(arg_forms: &[RispExp], env: &mut RispEnv) -> Result<RispExp, RispErr> {
    ...
    match test_eval {
        RispExp::Bool(b) => {
            ...
            let res_form = arg_forms.get(form_idx).ok_or(RispErr::InvalidTestForm)?;
            eval(res_form, env)
        }
        _ => Err(RispErr::UnexpectedTestForm),
    }
...
fn eval_list(list: &Vec<RispExp>, env: &mut RispEnv) -> Result<RispExp, RispErr> {
    let first_form = list.first().ok_or(RispErr::EmptyList)?;
